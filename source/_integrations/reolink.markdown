...
You'll need to configure your new camera/doorbell for you network. If you're using a wired LAN this is simple enough. Simply plug it in and let it use DHCP to set itself up. Once it's recognized by your network you can access it's configuration with a web browser.

If you prefer to use Wi-Fi, especially if you want to retrict it to a local IOT network, [here's a quick way to attach it to your network](https://www.reddit.com/r/reolink/comments/hjn5be/how_to_connect_wifi_without_an_app/). Simply create a QR code with the following XML string:
